...
var Er = N(() => {
  ...
  yr = (n, t) => {
    ...
    if (u != null) {
      ...
      let g = d * o, b = 0, y = g, T = g * 2, O = -1;
      c === "RGBA" ? (b = 0, y = g, T = g * 2, O = g * 3) : c === "RGB" ? (b = 0, y = g, T = g * 2) : c === "RBG" && (b = 0, T = g, y = g * 2);
      for (let U = 0; U < d; U++) for (let z = 0; z < o; z++) {
        let A = (n.data[b++] - h[0]) * m[0], I = (n.data[y++] - h[1]) * m[1], F = (n.data[T++] - h[2]) * m[2], D = O === -1 ? 255 : (n.data[O++] - h[3]) * m[3];
        u.fillStyle = "rgba(" + A + "," + I + "," + F + "," + D + ")", u.fillRect(z, U, 1, 1);
      }
    ...
    if (a != null) {
      ...
      let y = 4, T = 0, O = 1, U = 2, z = 3, A = 0, I = b, F = b * 2, D = -1;
      l === "RGBA" ? (A = 0, I = b, F = b * 2, D = b * 3) : l === "RGB" ? (A = 0, I = b, F = b * 2) : l === "RBG" && (A = 0, F = b, I = b * 2), u = a.createImageData(o, d);
      for (let k = 0; k < d * o; T += y, O += y, U += y, z += y, k++) u.data[T] = (n.data[A++] - g[0]) * h[0], u.data[O] = (n.data[I++] - g[1]) * h[1], u.data[U] = (n.data[F++] - g[2]) * h[2], u.data[z] = D === -1 ? 255 : (n.data[D++] - g[3]) * h[3];
    } else throw new Error("Can not access image data");
    return u;
  };
});
...
var Ir = N(() => {
  ...
  Kt = (n, t) => {
    ...
    let l = t.format !== void 0 ? t.format : "RGBA", m = t.tensorFormat !== void 0 && t.tensorFormat !== void 0 ? t.tensorFormat : "RGB", h = a * u, g = m === "RGBA" ? new Float32Array(h * 4) : new Float32Array(h * 3), b = 4, y = 0, T = 1, O = 2, U = 3, z = 0, A = h, I = h * 2, F = -1;
    l === "RGB" && (b = 3, y = 0, T = 1, O = 2, U = -1), m === "RGBA" ? F = h * 3 : m === "RBG" ? (z = 0, I = h, A = h * 2) : m === "BGR" && (I = 0, A = h, z = h * 2);
    for (let k = 0; k < h; k++, y += b, O += b, T += b, U += b) g[z++] = (n[y] + c[0]) / d[0], g[A++] = (n[T] + c[1]) / d[1], g[I++] = (n[O] + c[2]) / d[2], F !== -1 && U !== -1 && (g[F++] = (n[U] + c[3]) / d[3]);
    ...
    if (a) {
      ...
      if (d) return new Promise((g, b) => {
        ...
        if (!n || !T) return b();
        let O = new Image();
        O.crossOrigin = "Anonymous", O.src = n, O.onload = () => {
          y.width = O.width, y.height = O.height, T.drawImage(O, 0, 0, y.width, y.height);
...
async function Vr(n = {}) {
  ...
  var g, b, y, T, O, U, z = console.log.bind(console), A = console.error.bind(console), I = z, F = A, D = false, k = (e) => e.startsWith("file://");
  function w() {
    ...
    let e = function(r) {
      try {
        ...
        if (s === "load") {
          ...
          }, p == "print" && (I = t[p]), p == "printErr" && (F = t[p]));
  ...
  var j, ne, pe, B, W, re, me, v, K, He = false;
  function ee() {
    var e = ge.buffer;
    t.HEAP8 = j = new Int8Array(e), pe = new Int16Array(e), t.HEAPU8 = ne = new Uint8Array(e), new Uint16Array(e), t.HEAP32 = B = new Int32Array(e), t.HEAPU32 = W = new Uint32Array(e), re = new Float32Array(e), me = new Float64Array(e), v = new BigInt64Array(e), new BigUint64Array(e);
  }
  function he() {
    He = true, o ? U() : Ee.Ua();
  }
  function H(e) {
    throw F(e = "Aborted(" + e + ")"), D = true, e = new WebAssembly.RuntimeError(e + ". Build with -sASSERTIONS for more info."), O?.(e), e;
  }
  function q() {
  ...
  var ve = (e) => {
    ...
    for (s of i) typeof s == "bigint" ? ((w(), v)[S++ >>> 0] = 1n, (w(), v)[S++ >>> 0] = s) : ((w(), v)[S++ >>> 0] = 0n, (w(), me)[S++ >>> 0] = s);
  ...
  function ko(e, r, i, s, f) {
    for (r >>>= 0, f >>>= 0, Nt.length = 0, i = f >>> 3, s = f + s >>> 3; i < s; ) {
      var p;
      p = (w(), v)[i++ >>> 0] ? (w(), v)[i++ >>> 0] : (w(), me)[i++ >>> 0], Nt.push(p);
    }
  ...
  function Xo(e, r, i) {
    ...
    return e = Math.round(1e6 * e), (w(), v)[i >>> 3 >>> 0] = BigInt(e), 0;
  }
  var kt = [];
  function Zo(e, r, i) {
    ...
    for (var s; s = (w(), ne)[r++ >>> 0]; ) {
      var f = s != 105;
      i += (f &= s != 112) && i % 8 ? 4 : 0, kt.push(s == 112 ? (w(), W)[i >>> 2 >>> 0] : s == 106 ? (w(), v)[i >>> 3 >>> 0] : s == 105 ? (w(), B)[i >>> 2 >>> 0] : (w(), me)[i >>> 3 >>> 0]), i += f ? 8 : 4;
    }
    return lr[e](...kt);
  }
  var Qo = () => {
...
`))[0] == "Error" && s.shift(), jn(s);
  ...
  function Qn(e, r, i, s) {
    ...
    for (var f = 0, p = 0; p < i; p++) {
      ...
      for (var M = 0; M < S; M++) {
        ...
        X === 0 || X === 10 ? ((R === 1 ? I : F)(An(le)), le.length = 0) : le.push(X);
      }
      f += S;
    }
  ...
  var at, Kn, er, zt, tr, nr, rr, Ht, or, ar, x, Ye, sr, P, jt, _, ir, Vt, ur, fr, cr, Ee, ua = [V, fe, Sn, In, Bn, Ln, Pn, Dn, _n, Un, xn, Mn, Cn, Rn, Fn, Nn, $n, zn, Hn, Yn, Jn, qn, Xn, Zn, Qn], lr = { 886300: (e, r, i, s, f) => {
  ...
  }, 887124: () => typeof wasmOffsetConverter < "u" };
  function fa() {
...
var dt = N(() => {
  "use strict";
  lt();
  Zr = typeof location > "u" ? void 0 : location.origin, an = import.meta.url > "file:" && import.meta.url < "file;", ss = () => {
    if (true) {
      if (an) {
        let n = URL;
        return new URL(new n("ort.wasm.bundle.min.mjs", import.meta.url).href, Zr).href;
      }
...
var xe = N(() => {
  ...
  un = false, Tt = false, no = false, cs = () => {
    ...
    let o = n.wasmPaths, d = typeof o == "string" ? o : void 0, c = o?.mjs, l = c?.href ?? c, m = o?.wasm, h = m?.href ?? m, g = n.wasmBinary, [b, y] = await to(l, d, a > 1, !!g || !!h), T = false, O = [];
    if (t > 0 && O.push(new Promise((U) => {
      setTimeout(() => {
        T = true, U();
      }, t);
    })), O.push(new Promise((U, z) => {
      let A = { numThreads: a };
      if (g) A.wasmBinary = g;
      else if (h || d) A.locateFile = (I) => h ?? d + I;
      else if (l && l.indexOf("blob:") !== 0) A.locateFile = (I) => new URL(I, l).href;
      else if (b) {
        let I = Qr();
        I && (A.locateFile = (F) => I + F);
      }
      y(A).then((I) => {
        Tt = false, un = true, sn = I, U(), b && URL.revokeObjectURL(b);
      }, (I) => {
        Tt = false, no = true, z(I);
      });
    })), await Promise.race(O), T) throw new Error(`WebAssembly backend initializing failed due to timeout: ${t}ms`);
  }, $ = () => {
    if (un && sn) return sn;
...
var rn = N(() => {
  ...
  bs = (n, t) => {
    ...
    try {
      ...
      let [b, y] = ys(d), T = !!t?.enableGraphCapture, O = [], U = [], z = [], A = [], I = [];
      for (let D = 0; D < b; D++) {
        let [k, w, Q] = co(d, D);
        k === 0 && G("Can't get an input name."), h.push(k);
        let j = o.UTF8ToString(k);
        O.push(j), z.push(w === 0 ? { name: j, isTensor: false } : { name: j, isTensor: true, type: Ot(w), shape: Q });
      }
      for (let D = 0; D < y; D++) {
        let [k, w, Q] = co(d, D + b);
        k === 0 && G("Can't get an output name."), g.push(k);
        let j = o.UTF8ToString(k);
        U.push(j), A.push(w === 0 ? { name: j, isTensor: false } : { name: j, isTensor: true, type: Ot(w), shape: Q });
      }
      return Ge.set(d, [d, h, g, null, T, false]), [d, O, U, z, A];
    } catch (b) {
    ...
    let l = $(), m = l.PTR_SIZE, h = n[0], g = n[1], b = n[3], y = b, T, O;
    ...
    if (b === "gpu-buffer") {
      ...
      let A = n[2].mlTensor;
      O = We(ke(h), g);
      let I = l.webnnRegisterMLTensor;
      if (!I) throw new Error('Tensor location "ml-tensor" is not supported without using WebNN.');
      T = I(u, A, ke(h), g);
    } else {
      let A = n[2];
      if (Array.isArray(A)) {
        O = m * A.length, T = l._malloc(O), a.push(T);
        for (let I = 0; I < A.length; I++) {
          if (typeof A[I] != "string") throw new TypeError(`tensor data at index ${I} is not a string`);
          l.setValue(T + I * m, ae(A[I], a), "*");
        }
      } else {
        let I = l.webnnIsGraphInput, F = l.webnnIsGraphOutput;
        if (h !== "string" && I && F) {
          let D = l.UTF8ToString(o);
          if (I(u, D) || F(u, D)) {
            let k = ke(h);
            O = We(k, g), y = "ml-tensor";
            ...
            let j = await w(u, k, g);
            Q(j, new Uint8Array(A.buffer, A.byteOffset, A.byteLength)), T = j;
          } else O = A.byteLength, T = l._malloc(O), a.push(T), l.HEAPU8.set(new Uint8Array(A.buffer, A.byteOffset, O), T);
        } else O = A.byteLength, T = l._malloc(O), a.push(T), l.HEAPU8.set(new Uint8Array(A.buffer, A.byteOffset, O), T);
      }
    }
    let U = l.stackSave(), z = l.stackAlloc(4 * g.length);
    try {
      g.forEach((I, F) => l.setValue(z + F * m, I, m === 4 ? "i32" : "i64"));
      let A = l._OrtCreateTensor(ke(h), T, O, z, g.length, fo(y));
      A === 0 && G(`Can't create tensor for input/output. session=${u}, index=${d}.`), t.push(A);
    } finally {
      l.stackRestore(U);
    }
    ...
    let h = m[0], g = m[1], b = m[2], y = m[3], T = m[4], O = m[5], U = t.length, z = u.length, A = 0, I = [], F = [], D = [], k = [], w = [], Q = c.stackSave(), j = c.stackAlloc(U * l), ne = c.stackAlloc(U * l), pe = c.stackAlloc(z * l), B = c.stackAlloc(z * l);
    try {
      [A, I] = ro(d), _e("wasm prepareInputOutputTensor");
      for (let v = 0; v < U; v++) await lo(a[v], F, k, n, g[t[v]], t[v], T);
      for (let v = 0; v < z; v++) await lo(o[v], D, k, n, b[u[v]], U + u[v], T);
      Ue("wasm prepareInputOutputTensor");
      for (let v = 0; v < U; v++) c.setValue(j + v * l, F[v], "*"), c.setValue(ne + v * l, g[t[v]], "*");
      for (let v = 0; v < z; v++) c.setValue(pe + v * l, D[v], "*"), c.setValue(B + v * l, b[u[v]], "*");
      c.jsepOnRunStart?.(h), c.webnnOnRunStart?.(h);
      let W;
      W = await c._OrtRun(h, ne, j, U, B, z, pe, A), W !== 0 && G("failed to call OrtRun().");
      let re = [], me = [];
      _e("wasm ProcessOutputTensor");
      for (let v = 0; v < z; v++) {
        let K = Number(c.getValue(pe + v * l, "*"));
        if (K === D[v] || w.includes(D[v])) {
          re.push(o[v]), K !== D[v] && c._OrtReleaseTensor(K) !== 0 && G("Can't release tensor.");
          continue;
        }
        let He = c.stackSave(), ee = c.stackAlloc(4 * l), he = false, H, q = 0;
        try {
          c._OrtGetTensorData(K, ee, ee + l, ee + 2 * l, ee + 3 * l) !== 0 && G(`Can't access output tensor data on index ${v}.`);
          ...
          H = Ot(ve);
          let se = y?.outputPreferredLocations[u[v]];
          if (H === "string") {
      ...
      for (let [v, K] of await Promise.all(me)) re[v][2] = K;
      return Ue("wasm ProcessOutputTensor"), re;
    } finally {
      c.webnnOnRunEnd?.(h), c.stackRestore(Q), F.forEach((W) => c._OrtReleaseTensor(W)), D.forEach((W) => c._OrtReleaseTensor(W)), k.forEach((W) => c._free(W)), A !== 0 && c._OrtReleaseRunOptions(A), I.forEach((W) => c._free(W));
    }
  }, gt = (n) => {
    let t = $(), a = Ge.get(n);
  ...
  Eo = (n, t) => {
    ...
    async run(t, a, u) {
      ...
      Object.entries(t).forEach((y) => {
        let T = y[0], O = y[1], U = this.inputNames.indexOf(T);
        if (U === -1) throw new Error(`invalid input '${T}'`);
        o.push(O), d.push(U);
      });
      let c = [], l = [];
      Object.entries(a).forEach((y) => {
        let T = y[0], O = y[1], U = this.outputNames.indexOf(T);
